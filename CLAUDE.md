...
#### 1. Find the Project
```bash
# List all directories in ~/repos to find projects
ls -la ~/repos/ | grep "^d" | awk '{print $NF}' | grep -v "^\."

# If project name is ambiguous, list matches
ls -la ~/repos/ | grep -i "task"  # for "task templates"
```
...
PROJECT_PATH="$HOME/repos/$PROJECT_NAME"
...
```bash
# 1. Find project
ls -la ~/repos/ | grep -i task
# Found: task-templates

# 2. Create session
tmux new-session -d -s task-templates -c "$HOME/repos/task-templates"

# 3. Set up windows
tmux rename-window -t task-templates:0 "Claude-Agent"
tmux new-window -t task-templates -n "Shell" -c "$HOME/repos/task-templates"
tmux new-window -t task-templates -n "Dev-Server" -c "$HOME/repos/task-templates"
...
```bash
# Find project
ls -la ~/repos/ | grep -i "[project-name]"
...
tmux capture-pane -t [session]:[window] -S - -E - > \
  ~/repos/tmux-orchestrator/registry/logs/[session]_[role]_$(date +%Y%m%d_%H%M%S).log
...
```
~/repos/tmux-orchestrator/registry/
...
- **Location**: `./send-claude-message.sh` (run from project root)
